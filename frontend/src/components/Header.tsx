...
function Header() {
  const { userName, token, setToken, setShowCart, setStoreId } =
    useCommerceStore();
    ...
    <div className="header flex justify-between items-center px-6 py-4">
      ...
      <div className="nav flex gap-4 items-center min-h-10">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          {showProfile && (
            <div
              className="absolute cursor-pointer border px-1 -bottom-7 right-0 border-gray-300 bg-gray-300 shadow-lg"
              onClick={async () => {
                await setToken("");
                await setStoreId("");
                console.log(token);
